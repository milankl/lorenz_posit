...
# assume the attractor is the x,y plane represented by m^2 points
m = 100
Δx = 1/m     # distance of points on the attractor
xc = vec([x for x=linspace(0,1,m),y=linspace(0,1,m)])
yc = vec([y for x=linspace(0,1,m),y=linspace(0,1,m)])
zc = 0.5*ones(m^2)
